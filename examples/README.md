...
| [text-utils](text-utils/) | Python | The same three-capability shape in Python: `@tool` from type hints, `@action` with `Field`s, and a clock-driven trigger | `tools`, `actions`, `triggers` | `fire_trigger` | **Python 3.10+** on the machine running Astra (`runtimes = ["python"]`) |
