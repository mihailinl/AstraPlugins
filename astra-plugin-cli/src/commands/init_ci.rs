...
/// Which version of the generated caller `render()` writes (registry plan
/// AP-24; contract SCOPE-7, ID-73).
///
/// A generated workflow lives in an author's repository, where nobody reruns
/// `init-ci`, so whatever it compiles is supported for 12 months after the
/// first CLI release that stops writing it: the author audience and the
/// author-notify path it calls, if any, may not be retired before then. That
/// period needs a name for each shape of the file, and this is it. The first
/// line of every generated file carries it, so a caller found in the wild says
/// which template it is.
///
/// **Bump it when what the caller compiles changes** (an audience, a notify
/// path, a call), add the new version to `spec/init-ci-templates.yaml`, record
/// the release that stopped writing the old one there, and say so in the
/// CHANGELOG. A new version also waits for the contract MINOR that lists it
/// (SCOPE-1). C33 in `tools/check-registry-mirrors.py` is red on a bump the
/// list does not carry. A comment or an input default is not a new template.
pub const CALLER_TEMPLATE_VERSION: u32 = 1;
...
pub fn render(pin: &Pin, plugin_dir: &str, linux_packages: &str, tag_prefix: &str) -> String {
    format!(
        r#"# astra-plugin init-ci template {template}
...
jobs:
  release:
    ...
    # nothing to leak.
"#,
        template = CALLER_TEMPLATE_VERSION,
        tag = WORKFLOW_TAG,
        repo = WORKFLOW_REPO,
        file = WORKFLOW_FILE,
        sha = pin.sha,
...
mod tests {
    ...
    /// AP-24: the first line of a generated caller names its template
    /// version, the number `spec/init-ci-templates.yaml` and the registry's
    /// token file list. Read back as the file's own first line, from every
    /// shape `render()` is called with, so a header moved below a comment or
    /// spelled from a literal instead of the constant is red here.
    #[test]
    fn the_first_line_names_the_template_version() {
        let sha = "0".repeat(40);
        let want = format!("# astra-plugin init-ci template {CALLER_TEMPLATE_VERSION}");
        for (dir, packages, prefix) in [(".", "", "v"), ("plugins/dice", "libasound2-dev", "dice-v")] {
            let text = render(&pin(&sha), dir, packages, prefix);
            assert_eq!(
                text.lines().next(),
                Some(want.as_str()),
                "the generated caller's first line must be {want:?}; it starts:\n{}",
                text.lines().take(3).collect::<Vec<_>>().join("\n")
            );
            assert_eq!(
                text.matches("# astra-plugin init-ci template ").count(),
                1,
                "the template marker appears more than once"
            );
        }
    }
