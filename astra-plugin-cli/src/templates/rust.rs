...
[dependencies]
# Released SDK — works once published to crates.io. 0.6 is the first release
# whose HostClient attaches `x-session-token`; 0.5 and earlier get
# `unauthenticated` from the daemon on every host call, so do not relax this.
...
"##,
    ...
    async fn execute_action(&self, action_type: &str, _params_json: &str) -> ActionResult {
...
async fn main() -> anyhow::Result<()> {{
    astra_plugin_sdk::run({struct_name}).await
}}
"#
    )
}
