...
**状态：** 对文档格式和验证规则是规范性的。**已生效：** 这里描述
的签名链已经规定好，两端都已实现，并且从头到尾锚定——在把本文件中的
...
* **一个根密钥并不会给目录签名。** 它给 `trust.json` 签名，而后者委
  ...
  经不会再触发了。
* **客户端拿到的目录已经用它签了名。** 从 2026-09-20 起，注册表的签名器
  （`astra-registry/.github/workflows/sign.yml`）是客户端所读内容的唯一
  发布者。它用 `astra-index-2026a` 给 `index.json` 和 `revocations.json`
  签名，把它们提交到 `signed` 分支，并把这个分支里的文档部署到 Pages。
  2026-09-24 核实过：Pages 提供的 `index.json` 能对照与它一起提供的
  `trust.json`，在 `astra-index-2026a` 下通过验证，序列号为 52。守护进程的
  `classify_signature` 把目录仍可能以未签名状态到达的两种情形分得很清楚：
  `NoTrustAnchor` 意味着没有任何经过验证的 `trust.json` 到达这个构建，
  因此根本没有密钥可以用来校验签名；`NoSignatures` 意味着信任锚就在那里，
  而目录自己一个签名也没有。
* 提交在注册表 `main` 上的副本 —— `registry/v1/index.json` 和
  `registry/v1/revocations.json` —— 是有意携带 `"signatures": []` 的：
  "未签名"被明明白白地说了出来，而不是让一个缺失的成员和一个被剥离的成员
  无法区分。没有任何客户端读取它们；已签名的副本在 `signed` 分支和 Pages 上。
* **Pages 提供的撤回列表也已签名**，自 astra-registry 提交 `@FLAG7@`
  （@ARMED_DATE@）添加 `policy/pages-withdrawal-list.json` 起。由于
  `verify_revocations_document` 是严格的（§6.4），守护进程只在首次获取到一份
  签名有效的列表之前处于 `RevocationFreshness::NotEnforced`。从那以后它会强制
  执行撤回，§5.5 的 7 天封锁对它适用，而且它不会再解除。

下面描述的一切都是格式和算法本身，等剩下的那一环落地时，这些内容
不会有任何变化。根仪式已经执行过了，委托也已经签好了，客户端拿到的
`index.json` 在它的 `signatures` 数组里带着一个签名，所以这条链中属于
目录的那一半已经在用户的机器上产生了实际分量，撤回列表的那一半也一样：
Pages 提供的是已签名的列表。
