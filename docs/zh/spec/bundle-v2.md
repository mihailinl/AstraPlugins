...
`testdata/bundles/` 保存着那些被冻结的 `.astraplugin` 文件、
...
| `header-disagree` | §4.1 |
| `manifest-first-by-offset-only` | §13 第 6 步——中央目录的条目 0 |
| `manifest-first-by-index-only` | §4——偏移 0 处的本地头 |
...
关于 `manifest-first-by-offset-only` 和 `manifest-first-by-index-only`
的说明：一个 ZIP 有两种顺序——本地记录被写下来的顺序，以及中央目录
列出它们的顺序——而 §13 对两者都提出了要求，一次在第 2 步，一次在
第 6 步。这里其他每一个向量都让这两种顺序保持一致，而
`manifest-not-first` 是同时破坏了两者，所以一个只实现了其中一步的
校验器，仍然能通过这张表里其余的每一行。这两个向量各自只破坏一个，
方向相反，并且只有成对出现才构成一项检验。
...
每一个构件摘要都匹配，存在的每一个清单摘要也都匹配：被跳过的是那些
条目零按构造本身就不是一份以存储方式保存的清单文件的向量——也就是
恰好携带 `manifest_digest: null` 的那些记录。一个共享的 bug 可以让三个
