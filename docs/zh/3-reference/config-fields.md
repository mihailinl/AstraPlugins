...
每种类型还带有一个 **`icon_svg`** —— 完整的 SVG 标记，而不是路径字符串 —— 也就是该
步骤在画布上、面板中以及自身配置标题处所佩戴的图形。给它一个 `viewBox`，不要写
`width`/`height`，因为尺寸由 Astra 决定，而这三处各不相同；并使用 `currentColor`
绘制，使其跟随每处为图标着色的方式。它在渲染前会被净化：脚本、事件处理器和外部引用
会被移除而非拒绝。留空它，步骤依然会绘制，并回退到插件自身的图标，然后回退到通用的
插件标记 —— 所以图标的作用是把你的步骤和别人的区分开，而不是让它可见。
