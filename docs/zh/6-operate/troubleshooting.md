...
今天，信任链已经**从头到尾**锚定：根密钥已经存在，经根签名的 `trust.json`
委托了索引签名密钥，客户端拿到的目录和撤回列表都已经用它签了名。提交在
注册表 `main` 上的副本是有意携带 `"signatures": []` 的，没有任何客户端
读取它们。**`REVOCATIONS_STALE`** 表示这台机器上的撤回列表已超过 7 天：
联网之后，Astra 会获取一份新的。参见
