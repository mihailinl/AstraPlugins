...
没有被扣留的发布，会在账户得知之前就发布出去。所以**绝不要合并一行
...
绑定行的发布会被扣留一次，直到一位审核员批准它，即 `R_FIRST_BINDING`。并且从
切换起，`grandfathered` 上架中被扣留并获得批准的发布，会一直等到该上架完成绑定。
...
**两条路径，等待的东西不一样。** 从这里往下，直到
[自切换起的面板路径](#自切换起的面板路径)，描述的都是通过注册表 issue 表单
提出的申请，今天的首次上架就是这样完成的。从注册表切换起，发布改为通过
Astra 插件面板到达注册表，**通过所有自动检查的发布会立即发布**，并被标记为
未经 Astra 审核员审核。在那里，只有已有上架的易手才需要等待审核员。
...
**通过 issue 表单时，首次上架永远不属于上述情况之一。** 按定义它一定会被
保留给人来判断 —— 见下文 —— 所以在那里，"我的第一个插件要多久才能上架"
这个问题的答案是*bot 评论之后最多 48 小时*，而不是*几分钟*。在面板路径上，
自切换起，答案是几分钟：见[下文](#自切换起的面板路径)。

### 保留状态如何被解除

不需要你做任何事。通过 issue 表单时，一位维护者会在你的 issue 上评论
**`/approve`**，
...
三件事，而且这份清单不会在注册表已发布的政策没有变化的情况下增长：
...
通过 issue 表单时，有些发布通过了一切检查，却依然不会立即发布：
...
*2026-09-26 修订（注册表合约 3.0.0）。* 在这个日期之前，本页说首次上架总会
被保留给人来判断。通过 issue 表单时仍然如此，直到切换移除这个表单为止。
注册表自己的规则见 `astra-registry/docs/POLICY.md` §2.1 和 §3。

**通过所有自动检查的发布会立即发布**，首次上架也不例外。没有人批准它，也没有
发布延迟。注册表的检查就是全部的关卡，任何一项检查失败，发布依然会被拒绝。

**它会被标记为未经 Astra 审核员审核。** 每个版本一开始都是这样。审核员之后
可能会读某个版本，并把它标记为已审核。这个标记只属于那一个版本，所以你的
下一次发布又会从未审核开始。

**用户会看到警告。** 从加入这个标记的那个 Astra 版本起，Astra 在安装一个未
审核的版本之前会发出警告，由 Astra 的 AI 工具请求的安装也一样。用户在任何地方
开始更新到这样的版本时，Astra 也会发出警告，并且只有在用户确认了警告之后才会
应用这次更新。插件面板会在你插件的页面上显示这个标记。Astra 0.2.x，以及加入
这个标记之前的每一个 Astra 版本，既不显示标记，也不显示警告。

**`reviewed` 的意思是有一位审核员读过那个版本，仅此而已。** 它不是安全审查，
不是代码审计，不是背书，也不是沙箱。之后的公告(advisory)、撤回版本(yank)或
下架(delist)永远优先于它。
...
| 事件 | 代码 |
|---|---|
| 仓库或身份发生了变化 | `R_IDENTITY_CHANGED` |
| 已有上架中第一个带有绑定行的发布 | `R_FIRST_BINDING` |
| 绑定行携带的令牌与该上架绑定时的令牌不同 | `R_BINDING_CHANGED` |

每一种都会把另一方的代码，或者把上架交到另一个账户手里，作为一次更新送到
已经在运行它的人那里。审核员在面板里批准它，之后它还要等过注册表公布的那些
时间窗口。获得批准的发布依然会被发布为未审核，因为批准不是审核。
...
上架不是一次安全审查，被标记为已审核的版本也不是：那只说明有一位审核员
读过那个版本，仅此而已。注册表自己的政策也明确这么说：一项权限决定的是守护进程会*为*插件做什么，而完全不涉及插件的
