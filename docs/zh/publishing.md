...
下面是通过 issue 表单时的简版，今天的首次上架就是这样完成的：
...
**通过 issue 表单时，首次上架总是会被保留给人来判断** —— 这是那里恰好需要
人工介入的三种事件之一，另外两种是新申请的高风险权限，以及仓库变更。48 小时
是这三种情况共同公布的 SLA。

**在 Astra 插件面板上，自注册表切换起，它不会被保留。** 通过所有自动检查的
发布会立即发布，并被标记为未经 Astra 审核员审核。从加入这个标记的那个 Astra
版本起，在审核员审核那个版本之前，用户会看到一条警告；更早的 Astra 版本什么
都不显示。在那里，只有已有上架的易手才需要等待审核员。参见
[申请上架 §自切换起的面板路径](5-publish/get-listed.md#自切换起的面板路径)。
...
没有沙箱。上架不是安全审查，被审核员标记为已审核的版本也不是：那只说明有
一位审核员读过那个版本，仅此而已。注册表自己的政策也是这么说的。
