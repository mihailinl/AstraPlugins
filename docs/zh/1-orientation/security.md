...
### 信任链已经规范化并实现，并且从头到尾锚定

根密钥(root key)已经存在，它们之下的委托关系也存在，**目录本身的签名**
也存在了，Pages 提供的撤回列表同样已签名。具体来说
...
- **而且 Pages 提供的撤回列表是已签名的**，自 astra-registry 提交
  `@FLAG7@`（@ARMED_DATE@）起。一次默认构建从首次获取到签名有效的列表起就
  强制执行撤回，此后只要它持有的列表超过 7 天，就会拒绝新的安装
  （[`spec/registry-index.md` §5.5](../spec/registry-index.md)）。

这里没有对你今天还没有的保证做出任何承诺。目录的签名和撤回列表现在都已经
对用户产生分量。
...
| 一个已被撤回、但已经装上的版本 | **有防御。** Pages 提供的撤回列表是已签名的，一次默认构建从首次获取到签名有效的列表起就会强制执行它 |
