...
### 信任链已经规范化并实现，但在撤回这一环上还差一步

根密钥(root key)已经存在，它们之下的委托关系也存在，**目录本身的签名**
也存在了。用户的机器还拿不到的，是一份已签名的撤回列表。具体来说
...
- **客户端拿到的目录是已签名的。** 从 2026-09-20 起，注册表的签名器用
  `astra-index-2026a` 给 `index.json` 签名并把它部署到 Pages，所以一次默认
  构建有目录签名可以校验。提交在注册表 `main` 上的副本依然携带
  `"signatures": []`，这是有意的，而且没有任何客户端读取它们；
- **但 Pages 仍然提供未签名的撤回列表**，直到注册表在那里启用已签名的
  列表为止；而由于撤回列表(withdrawal list)是被严格校验的，一份未签名的
  撤回列表会被拒绝，所以**失效强制机制尚未生效**。

这里没有对你今天还没有的保证做出任何承诺。目录的签名现在就已经对用户
产生分量；撤回则要等已签名的列表出现在 Pages 上之后。
...
| 一个恶意或被攻陷的注册表提供了不同的字节内容 | *在设计上*是有防御的 —— 索引对摘要进行反签名，守护进程会重新哈希 —— 而且根已经就绪（`registry/v1/root.json`，与编译进守护进程的是同样的两个密钥）。从 2026-09-20 起，客户端拿到的目录已在它们之下签名 |
| 一个已被撤回、但已经装上的版本 | 已有规范；今天尚未强制执行，因为 Pages 仍然提供一份未签名的撤回列表，而未签名的列表会被拒绝 |
