...
リリースも認証しないので、自分で発行していないバインディング行は決して
マージしないでください。それが何を記録し、どれだけ有効で、名前の変更が何を
もたらすかは
[掲載してもらう — リポジトリをバインドする](5-publish/get-listed.md#リポジトリをバインドする)
にあります。
...
にあります。issue フォームの場合、要約すると次のとおりです。最初の掲載は
今日、この方法で行います。
...
**issue フォームでは、最初のリスト掲載は必ず人による保留になります** —
これは、新たに要求された高リスクパーミッションおよびリポジトリの変更と
並んで、そこで人の判断が必要なちょうど 3 つのイベントのうちの 1 つです。
48 時間はそのすべてに対して公表されている SLA です。

**レジストリのカットオーバー以降、Astra のプラグインパネルでは保留されません。**
自動チェックをすべて通過したリリースはすぐに公開され、Astra のモデレーターが
レビューしていないという印が付きます。印に対応した Astra のリリース以降、
モデレーターがそのバージョンをレビューするまで、ユーザーには警告が表示
されます。それより古い Astra のリリースは何も表示しません。そこでモデレーター
を待つのは、すでに存在する掲載の持ち主が替わる場合だけです。
[リストに掲載してもらう §カットオーバー以降のパネル経路](5-publish/get-listed.md#カットオーバー以降のパネル経路)
を参照してください。
...
です。2026-09-20 以降、クライアントに配信されるカタログはその鍵で署名されて
います。レジストリの `main` にコミットされたコピー、`registry/v1/index.json`
と `revocations.json` は意図的に `"signatures": []` を持っており、これを読む
クライアントはありません。**まだ欠けているリンクは Pages 上の署名済みの撤回
リストです**。そのため失効はまだ強制されません。ここでは、まだ実装されていない保証を約束
...
リスト掲載は安全性のレビューではありません。モデレーターがレビュー済みの印を
付けたバージョンも同じです。その印が言うのは、モデレーターがそのバージョンを
読んだということだけです。レジストリ自身のポリシーもそう明言しています。
