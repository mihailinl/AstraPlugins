...
報告されるピンは `plugin-release/v1` が指すコミットでなければならず、それが何かを
答えられるのはリモートだけです: `git ls-remote
https://github.com/mihailinl/AstraPlugins.git refs/tags/plugin-release/v1
'refs/tags/plugin-release/v1^{}'` — peel 済みの `^{}` 行があればそちらを取ります。
ピンが食い違っていたら、上の行で `master` から入れ直し、`init-ci` を再実行して
ください。再実行しても安全です。入力値は保持したまま、ピンだけを書き換えます。
既存のファイルはその場で修復されないため、既存の `release.yml` は再実行するまで
古い SHA を保持し続けます。この節の名前になっている失敗は、いまや日付の付いた
歴史です: `plugin-release/v1` は 2026-08-11 から 2026-08-19 まで注釈付きタグ
であり、コミット `5b8ab22` より古いビルドは GitHub が拒否するそのタグ
*オブジェクト* `dc1a044876926e9cf1170f034e2eab533ec07641` を報告していました。
今日このタグは軽量タグなので、その SHA を報告できるビルドはそもそも存在しません。
