> **これは翻訳です。** 正式な情報源は [docs/en](../en/parity.md) です。内容に食い違いがある場合は英語版が優先されます。英語版のページは `tools/parity/gen.py` が生成しています。この翻訳は生成物ではなく手作業で保守されているスナップショットなので、英語版が再生成されたら手作業で訳し直してください。
