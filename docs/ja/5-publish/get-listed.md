...
認証しません** — 何にも保留されないリリースは、アカウントに知らされる前に
...
解け、ペナルティはありません。バインディング行を持つ最初のリリースは、
モデレーターが承認するまで一度保留されます(`R_FIRST_BINDING`)。そして
カットオーバー以降、`grandfathered` の掲載のリリースのうち、保留されて承認
されたものは、掲載がバインドされるまで待機します。
...
**経路は 2 つあり、待つものが違います。** この下から
[カットオーバー以降のパネル経路](#カットオーバー以降のパネル経路) までは、
レジストリの issue フォームを通じた申請を説明しています。最初の掲載は今日、
この方法で行います。レジストリのカットオーバー以降は、リリースは代わりに
Astra のプラグインパネルを通じてレジストリに届き、**自動チェックをすべて
通過したリリースはすぐに公開されます**。そのリリースには、Astra の
モデレーターがレビューしていないという印が付きます。そこでモデレーターを
待つのは、すでに存在する掲載の持ち主が替わる場合だけです。
...
**issue フォームでは、最初のリスト掲載はこれらのいずれにも該当しません。**
それは定義上、人による保留になります — 下記参照 — そのためそこでの
「最初のプラグインが掲載されるまでどれくらいかかるか」の答えは、*数分*
ではなく、*bot がコメントしてから最大 48 時間*です。カットオーバー以降の
パネル経路では、答えは数分です。[下記](#カットオーバー以降のパネル経路)
を参照してください。

### 保留はどう解除されるか

あなたに要求されることは何もありません。issue フォームでは、メンテナーが
あなたの issue に **`/approve`** とコメントし、それによって取り込み全体が、その時点での
...
### issue フォームで人を必要とすること

3 つのイベントで、レジストリの公表されたポリシーの変更なしにこのリストが
増えることはありません:
...
issue フォームでは、一部のリリースはすべてを通過してもすぐには公開
されません:
...
*2026-09-26 改訂(レジストリ契約 3.0.0)。* この日より前、このページは
最初のリスト掲載は必ず人による保留になると述べていました。issue フォーム
では今もそうです。カットオーバーでフォームがなくなるまでは。レジストリ
自身のルールは `astra-registry/docs/POLICY.md` の §2.1 と §3 です。

**自動チェックをすべて通過したリリースは、すぐに公開されます。** 最初の
リスト掲載も同じです。誰も承認せず、公開の遅延もありません。レジストリの
チェックがゲートのすべてであり、失敗したチェックがあれば、リリースは今も
拒否されます。

**そのリリースには、Astra のモデレーターがレビューしていないという印が
付きます。** どのバージョンもこの状態から始まります。モデレーターが後で
あるバージョンを読み、レビュー済みの印を付けることがあります。印はその
1 つのバージョンだけのものです。だから次のリリースは、また未レビューから
始まります。

**ユーザーには警告が表示されます。** 印に対応した Astra のリリース以降、
Astra はレビューされていないバージョンをインストールする前に警告します。
Astra の AI ツールが求めたインストールも同じです。また、ユーザーがそうした
バージョンへの更新を始めるところではどこでも警告し、ユーザーが警告を確認して
はじめて更新を適用します。プラグインパネルは、あなたのプラグインのページに
印を表示します。Astra 0.2.x と、印に対応するリリースより前のすべての Astra
リリースは、印も警告も表示しません。

**`reviewed` が意味するのは、モデレーターがそのバージョンを読んだという
ことだけです。** セキュリティレビューでも、コード監査でも、推薦でも、
サンドボックスでもありません。後から出た勧告、yank、掲載取り下げは、
常にこの印より優先されます。

**すでに存在する掲載の持ち主が替わる場合だけが、モデレーターを待ちます:**

| イベント | コード |
|---|---|
| リポジトリまたはアイデンティティが変わった | `R_IDENTITY_CHANGED` |
| 既存の掲載で、バインディング行を持つ最初のリリース | `R_FIRST_BINDING` |
| バインディング行が、掲載がバインドされているものとは別のトークンを持つ | `R_BINDING_CHANGED` |

どれも、別の当事者のコードを出荷するか、掲載を別のアカウントに渡すもの
であり、しかもそれをすでに使っている人々への更新として届きます。
モデレーターがパネルでそれを承認し、その後、レジストリが公表している
待機時間を待ちます。承認されたリリースも、未レビューとして公開されます。
承認はレビューではないからです。
...
リスト掲載は安全性のレビューではありません。レビュー済みの印が付いた
バージョンも同じです。その印が言うのは、モデレーターがそのバージョンを
