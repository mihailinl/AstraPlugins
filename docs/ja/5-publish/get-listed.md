...
**Astra は README の画像を表示せず、その alt テキストだけを表示します。**
画像があるはずの場所に、ストアは角括弧の中のテキストを表示します。つまり
そのテキストが、読者があなたのスクリーンショットについて知ることのすべて
であり、alt テキストが空の画像 `![](docs/editor.png)` は何も表示しません。
alt テキストは、ファイル名や「スクリーンショット」という語ではなく、読んで
ほしい文として書いてください:
...
レジストリはあなたのリストを導出する際に 3 つのルールを適用します。最初の
2 つは、この変更より前の Astra リリースが何を表示するかを決めます。それらの
リリースは GitHub でホストされた画像をまだ表示するからです:
