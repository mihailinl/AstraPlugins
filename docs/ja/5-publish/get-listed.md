...
**署名チェーンは、カタログと撤回リストを通して端から端まで固定されて
います。** 正確には、各部分は検証可能です:
...
- **クライアントに配信されるカタログは署名されています。** 2026-09-20
  ...
  Pages が配信する撤回リストも、astra-registry のコミット `@FLAG7@`
  (@ARMED_DATE@)以降は署名されているので、失効は強制されています。
...
実行され、bot が応答し、リスト掲載はコミットされ、レジストリはすでに
インストールされたコピーからバージョンを撤回できます。
