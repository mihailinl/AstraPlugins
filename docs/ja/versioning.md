...
3 つの SDK のバージョンは意図的にそろえていません。1 つの `sdk-v<VERSION>`
タグは、各パッケージ自身のマニフェストにあるバージョンをそのまま公開し、
`astra-plugin-macros` はバージョン指定でそれに依存する Rust SDK **より先に**
公開されます。それが今日どの番号なのかは、このページには書きません。以前は
現在のトレインを名指ししていて、`sdk-v0.7.1` が出たあとも 1 か月間
`sdk-v0.7.0` を名指しし続けました。だから番号は 1 か所にだけ置きます。README
の [Publication state](../../README.md#publication-state) の表です。そこでは
カップリング C23 が *In this tree* 列をマニフェストに、C23b が *Published* 列を
crates.io・PyPI・npm に照合します。
...
各 SDK が公開するプロトコルメッセージ — Rust の `astra_plugin_sdk::proto::*`、
Python の `_pb2` メッセージクラス、TypeScript SDK が読み込むディスクリプタ —
は `proto/plugin.proto` から生成されます。これはデーモン自身のスライスで、
デーモンのプロトコルが増えるたびに再同期されます。これらのバージョン規則:

| 生成された型への変更 | 枠 |
| --- | --- |
| フィールドまたはメッセージの追加 | パッチ — 追加 |
| フィールドの削除・改名・番号変更・型変更 | マイナー — 破壊的変更 |

**Rust でパッチの約束が及ぶ範囲:** `..Default::default()` を使ってメッセージを
構築すること、または `Default::default()` で作ってからフィールドを代入すること
です。網羅的な構造体リテラル — すべてのフィールドを列挙し `..` を持たないもの
— は対象外で、新しいフィールドが 1 つ増えると `E0063 missing fields` で壊れます。
`#[non_exhaustive]` な列挙型に対する `match` が対象外なのと同じです。
`astra-plugin new` は `..Default::default()` の形を書き出します。Python の
メッセージクラスはキーワード引数を受け取り、TypeScript SDK はディスクリプタを
実行時に読むので、この 2 つでは新しいフィールドは何も壊しません。

新しいフィールドがマイナーではない理由: デーモンのスライスを同期するたびに
フィールドが増えます — `sdk-v0.7.1` から 0.7.2 までの間に、既存のメッセージに
37 個 — ので、フィールドごとにマイナーにすると同期のたびにマイナーになり、
マイナー枠は「あなたのコードがコンパイルできなくなるかもしれない」という意味を
失います。生成された構造体に `#[non_exhaustive]` を付けて規則をコンパイラに
任せない理由: それはクレートの外からの `..Default::default()` も禁じるので、
明日の破壊から守るために今日すべてのプラグインのリテラルを壊すことになります。
ワイヤ互換性はそもそもこの規則の役目ではなく、上で述べたプロトコル整数の役目
です。SDK が手で書く型は通常の規則のままで、だから `Invocation` は
`#[non_exhaustive]` です。
