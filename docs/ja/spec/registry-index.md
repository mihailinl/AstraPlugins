...
**発効済み:** ここに記述されている署名チェーンは仕様化され、両端で実装され、
端から端まで固定されています — このファイル内の
...
* **Pages が配信する撤回リストも署名されています。** これは
  `policy/pages-withdrawal-list.json` を追加した astra-registry のコミット
  `@FLAG7@`(@ARMED_DATE@)以降のことです。`verify_revocations_document` が
  厳格である(§6.4)ため、デーモンが `RevocationFreshness::NotEnforced` のまま
  でいるのは、署名が有効なリストを一度取得するまでだけです。それ以降は撤回を
  強制し、§5.5 の 7 日間のブロックが適用され、元に戻ることはありません。
...
マシン上で重みを持っており、撤回リストの半分も同様です: Pages は署名済みの
リストを配信しています。
