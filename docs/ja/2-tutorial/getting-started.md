...
表示されるピンを `git ls-remote
https://github.com/mihailinl/AstraPlugins.git refs/tags/plugin-release/v1
'refs/tags/plugin-release/v1^{}'` の結果と比べてください — peel 済みの `^{}`
行があればそちらを取ります。この比較がバグの残りすべてです:
`plugin-release/v1` は 2026-08-11 から 2026-08-19 まで注釈付きタグで、
`5b8ab22` より古いビルドは代わりにそのタグオブジェクト
`dc1a044876926e9cf1170f034e2eab533ec07641` を表示していました。今このタグは
軽量タグなので、その SHA を表示するものはもうありません。
詳しい説明:
