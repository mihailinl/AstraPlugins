...
今日、信頼チェーンは**端から端まで**固定されています: ルートキーは存在し、
root 署名済みの `trust.json` はインデックス署名キーに委譲し、クライアントに
配信されるカタログと撤回リストはどちらもその鍵で署名されています。レジストリ
の `main` にコミットされたコピーは意図的に `"signatures": []` を持っており、
これを読むクライアントはありません。**`REVOCATIONS_STALE`** は、このマシンの
撤回リストが 7 日より古いことを意味します: ネットワークに接続すれば、Astra
が新しいリストを取得します。
