...
今日、信頼チェーンは**撤回のところで 1 リンク分だけ**足りません: ルート
キーは存在し、root 署名済みの `trust.json` はインデックス署名キーに委譲し、
クライアントに配信されるカタログはその鍵で署名されています。レジストリの
`main` にコミットされたコピーは意図的に `"signatures": []` を持っており、
これを読むクライアントはありません。Pages がまだ配信していないのは署名済み
の撤回リストなので、失効は強制されません。
