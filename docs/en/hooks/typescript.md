...
# TypeScript SDK — hook table

Protocol **1**. Every hook the TypeScript SDK can bind, and whether it does. Generated from [`spec/hooks.yaml`](../../../spec/hooks.yaml); the cross-language view is [`parity.md`](../parity.md).
...
- `TtsSynthesizeStream` — plugin.ts registers an entry that answers UNIMPLEMENTED, which is protocol-identical to absent.
