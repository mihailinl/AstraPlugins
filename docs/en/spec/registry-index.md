...
| `issued_at` | `YYYY-MM-DDTHH:MM:SSZ` | optional: stamped **at signing**, absent in the committed tree |
| `expires_at` | same | optional: `issued_at + 30 days`, stamped with it |
| `plugins` | array of `signed.plugins[]` | required: one record per listed plugin, sorted by `id` |
...
A plugin record is `signed.plugins[]`, and it and every object inside it have a
table of their own at the end of this section. The full JSON Schema is
`astra-registry/schema/index-v1.json`; it is `additionalProperties: false` and is
the authority on the field list.
...
**Every member, object by object.** Each table below is one object, named in the
line above it: `[]` is an element of an array, `<…>` a value in a map. A rule
begins with **required**, **optional** or **required when**, and those words are
the schema's `required` lists, not a census of today's catalogue — a member every
listing happens to carry is still optional if the schema says so, and a verifier
must accept its absence. Every object with a table is closed
(`additionalProperties: false`); the one open object is `permissions`. C21 in
`tools/check-registry-mirrors.py` fails when these tables, the `signed` table
above, and `astra-registry/schema/index-v1.json` at the registry's `main`
disagree about a member or its requiredness, in either direction.
