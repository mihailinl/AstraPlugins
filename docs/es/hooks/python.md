> **Traducción.** Esta es una traducción. Si hay alguna discrepancia, [`docs/en`](../../en/hooks/python.md) es la referencia autorizada. La página en inglés está GENERADA por `tools/parity/gen.py` a partir de `spec/hooks.yaml` — esta traducción es una instantánea mantenida a mano, no otra copia generada.

# SDK de Python — tabla de hooks

Protocolo **1**. Cada hook que el SDK de Python puede vincular, y si lo
hace. Generado a partir de
...
Ninguno. El SDK de Python vincula cada hook de la spec.
</content>
