> **Traducción.** Esta es una traducción. Si hay alguna discrepancia, [`docs/en`](../../en/hooks/rust.md) es la referencia autorizada. La página en inglés está GENERADA por `tools/parity/gen.py` a partir de `spec/hooks.yaml` — esta traducción es una instantánea mantenida a mano, no otra copia generada.

# SDK de Rust — tabla de hooks

Protocolo **1**. Cada hook que el SDK de Rust puede vincular, y si lo
...
Ninguno. El SDK de Rust vincula cada hook de la spec.
</content>
