> Перевод. Источник истины — [docs/en](../../en/reference/parity.md); при расхождении верна английская версия. Английская страница сгенерирована `tools/parity/gen.py` из `spec/hooks.yaml`; эта переводная копия не генерируется автоматически и может отстать при изменении спецификации.
