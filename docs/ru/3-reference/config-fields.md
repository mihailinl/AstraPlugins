...
Каждый тип несёт ещё и **`icon_svg`** — целую SVG-разметку, а не строку пути, —
глиф, который шаг носит на холсте, в палитре и в заголовке своих настроек.
Задайте `viewBox` и не указывайте `width`/`height`, потому что размер выбирает
Astra, а эти три поверхности разные, и рисуйте через `currentColor`, чтобы глиф
следовал за тем, чем каждая поверхность подкрашивает свои иконки. Разметка
очищается перед отрисовкой: скрипты, обработчики событий и внешние ссылки
вырезаются, а не отвергаются. Оставьте её пустой — шаг всё равно нарисуется,
откатившись к собственной иконке плагина, а затем к общей отметке плагина.
Значит, иконка отличает ваш шаг от чужих, а не делает его видимым.
