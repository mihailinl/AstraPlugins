...
| `@action("label", icon_svg="…")` | Шаг в редакторе команд, и глиф, который он там носит |
| `@trigger("label", icon_svg="…")` | Тип триггера, и его глиф |
...
**Глиф, который носит шаг.** `icon_svg` у `@action` или `@trigger` — это то, что
пользователь видит на холсте, в палитре и в заголовке настроек шага. Передавайте
целую SVG-разметку, а не строку пути: задайте `viewBox` и не указывайте
`width`/`height` (размер выбирает Astra, а эти три поверхности не одного
размера) и рисуйте через `currentColor`, чтобы глиф следовал за тем, чем каждая
поверхность подкрашивает свои иконки. Разметка проходит очистку перед отрисовкой
— скрипты, обработчики событий и внешние ссылки вырезаются, а не отвергаются, —
поэтому оставайтесь в пределах простого штрихового рисунка. Не задавайте её, и
шаг всё равно нарисуется: он откатится к собственной иконке плагина, а затем к
общей отметке плагина.
