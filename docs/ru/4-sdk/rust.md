...
`astra-plugin-sdk` 0.7.2. Одна зависимость, и всё, во что раскрываются
...
| `#[action(label = "…", icon = "…")]` | Шаг в редакторе команд, и глиф, который он там носит |
...
**Глиф, который носит шаг.** `icon` у `#[action]` и `with_icon_svg` у собранного
вручную `ActionTypeDef` или `TriggerTypeDef` — это то, что пользователь видит на
холсте, в палитре и в заголовке настроек шага. Передавайте целую SVG-разметку, а
не строку пути: задайте `viewBox` и не указывайте `width`/`height` (размер
выбирает Astra, а эти три поверхности не одного размера) и рисуйте через
`currentColor`, чтобы глиф следовал за тем, чем каждая поверхность подкрашивает
свои иконки. Разметка проходит очистку перед отрисовкой — скрипты, обработчики
событий и внешние ссылки вырезаются, а не отвергаются, — поэтому оставайтесь в
пределах простого штрихового рисунка. Не задавайте её, и шаг всё равно
нарисуется: он откатится к собственной иконке плагина, а затем к общей отметке
плагина.
...
Триггер, который вы зажигаете через `ctx.host()`, пока обрабатываете вызов от
Astra, приписывается этому вызову. Поэтому всё, что он вызовет, попадёт именно в
тот разговор, который человек сейчас перед собой видит. **Вам для этого писать
нечего**, и уже разосланная идиома продолжает работать без изменений:
...
Именно поэтому Rust ограничивает *дескриптор*, а не пользуется task-local:
`tokio::task_local!` не переживает `tokio::spawn`, а образцовый плагин как раз
спавнит. Клонируйте `ctx` или `ctx.host()` сколько угодно — привязку несут оба.
...
разговоре. Сюда попадают `astra_plugin_sdk::ctx()`, хост, сохранённый в
`on_start`, и голый `std::thread`. Это верный ответ, а не ухудшенный: не тот
разговор хуже, чем никакой.

У корневого события `testing::FiredTrigger::caused_by` равен `None`, так что
ваши тесты отличают одно от другого.
