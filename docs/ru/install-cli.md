...
**`cargo install astra-plugin-cli` станет одним из путей, только когда CLI
появится на crates.io, а до 0.4.0 включительно его там не было ни разу.** Крейт
зависел от завендоренного `astra-plugin-manifest` только по пути, а
`cargo publish` отвергает такое с *all dependencies must have a version
requirement specified* — поэтому релизы `cli-v0.2.1`, `cli-v0.3.0` и
`cli-v0.4.0` выпустили бинарники и ничего не загрузили на crates.io. Теперь у
зависимости есть версия, и релиз CLI загружает сначала `astra-plugin-manifest`,
а затем сам CLI. Пока `https://index.crates.io/as/tr/astra-plugin-cli` отвечает
`404`, а не `200`, берите бинарник или соберите из исходников.
...
Сообщённое закрепление обязано быть тем коммитом, на который указывает
`plugin-release/v1`, и сказать, какой это коммит, может только удалённый
репозиторий: `git ls-remote https://github.com/mihailinl/AstraPlugins.git
refs/tags/plugin-release/v1 'refs/tags/plugin-release/v1^{}'` — берите
снятую (`^{}`) строку, если она есть. Если закрепление расходится,
...
держит устаревший SHA, пока вы не перезапустите её. Отказ, по которому
назван этот раздел, теперь история, и она датирована: с 2026-08-11 по
2026-08-19 `plugin-release/v1` был аннотированным тегом, и сборка старше
коммита `5b8ab22` сообщала его *объект тега*
`dc1a044876926e9cf1170f034e2eab533ec07641`, который GitHub отклоняет, —
сегодня тег лёгкий, так что сообщить этот SHA не может никакая сборка.
