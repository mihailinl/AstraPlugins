...
и удостоверяет его, и вы один раз отправляете его в
панели — один раз, навсегда.
...
| Просьба к мейнтейнеру собрать его за вас | Никто не собирает ваш плагин, кроме собственного CI вашего репозитория. Другого сборщика нет |
| Описание плагина реестру где-либо, кроме страницы отправки в панели | Реестр действует по заявке, сделанной в панели, с входом в аккаунт, для репозитория, привязанного к вашему аккаунту. Другой двери нет. См. [Отправка](#8--отправьте-заявку-один-раз-навсегда) |
...
**[установку CLI](install-cli.md)**. Теперь есть готовые бинарники —
скачайте архив для Linux или Windows, сверьте его с `SHA256SUMS.txt`, и
никакой тулчейн не нужен. Сборка из исходников по-прежнему работает и
остаётся путём для macOS и ARM Linux. `cargo install astra-plugin-cli` —
вообще не путь; та страница объясняет, почему.
...
**Проверьте напечатанный SHA, прежде чем продолжать.** Он обязан быть тем
коммитом, на который `plugin-release/v1` указывает сегодня, и сказать, какой
это коммит, может только удалённый репозиторий: `git ls-remote
https://github.com/mihailinl/AstraPlugins.git refs/tags/plugin-release/v1
'refs/tags/plugin-release/v1^{}'` — оба refspec, и снятая (`^{}`) строка
побеждает, если удалённый её прислал; ровно эту пару спрашивает и сам
`init-ci`. Если ваше закрепление расходится, перезапустите строку
`cargo install` из [установки CLI](install-cli.md), затем запустите
`astra-plugin init-ci` снова — она перепишет закрепление и сохранит ваши
параметры. Ничто не чинится на месте, так что существующий `release.yml`
держит устаревший SHA, пока вы его не перезапустите. Остальное — история, и
она датирована: с 2026-08-11 по 2026-08-19 `plugin-release/v1` был
аннотированным тегом, CLI старше коммита `5b8ab22` закрепляла его объект
тега `dc1a044876926e9cf1170f034e2eab533ec07641` там, где GitHub требует
коммит, и это именно тот баг, что сломал первый релиз реального автора с
`invalid value workflow reference` ещё до старта любой задачи; сегодня тег
лёгкий, так что этот SHA больше не печатает ни одна сборка.
...
## 4 · Опубликуйте, публично — с файлом владения
...
В первых трёх строках нет ничего особенного — это обычный репозиторий. Но
обратите внимание, чем это *не* является: запушить это — не значит
опубликовать плагин, и остановиться здесь — именно то, на чём споткнулись две
реальные заявки, из-за которых появилась эта страница. Опубликованным плагином
его делает тег на следующем шаге.

**Строка привязки — это доказательство владения, и она не необязательна.**
Между пушем и `init-ci` войдите на https://astra.minice.ai/plugins в аккаунт
Minice, который будет публиковать, и выпустите токен привязки для
`you/dice-roller` — панель ищет репозиторий на GitHub, поэтому его нужно
запушить заранее. `init-ci --binding` записывает токен первой строкой
`.well-known/astra-plugin-owner` в корне вашего репозитория; закоммиченная в
ветку по умолчанию, она то, по чему реестр узнаёт, какой аккаунт говорит от
имени этого репозитория, — единственное, чего не может сказать удостоверение
сборки. Пропустите её — и первая заявка получит отказ `B_UNBOUND`.

Когда на следующем шаге вы поставите тег, `astra-plugin check --tag v0.1.0`
прочитает строку из коммита тега, как это сделает реестр. Токен публичен и не
аутентифицирует ни один релиз, поэтому никогда не мёржите строку привязки,
которую выпустили не вы. Что он фиксирует, сколько живёт и что с ним делает
переименование — в
[Попасть в каталог — Привяжите репозиторий](5-publish/get-listed.md#привяжите-репозиторий).
...
Из этого списка вердикт привязки — та проверка, которую решает ваша
собственная работа, и вы её сделали на
[шаге 4](#4--опубликуйте-публично--с-файлом-владения). Остальное следует из
того, что вы затегировали релиз, который собрал workflow.

## 8 · Отправьте заявку, один раз, навсегда

**Прежде чем запускать это**, прочитайте строку привязки из вашего тега ровно
так, как её прочтёт реестр. Это единственная проверка на этой странице, которую
можно не пройти, сделав всё остальное правильно:
...
`publish` открывает в браузере **страницу отправки** панели с уже заполненными
репозиторием и тегом. Он ничего не загружает и не хранит учётных данных — нет
`astra-plugin login`, нет токена в истории оболочки, нечего интегрировать с
keyring'ом. Страница ничего не отправляет, пока этого не сделаете вы, войдя в
аккаунт Minice, к которому привязан ваш репозиторий. `--print-url` печатает
ссылку вместо открытия браузера:
...
Заявка несёт **два факта**: ваш репозиторий-источник (`you/dice-roller`) и тег
релиза (`v0.1.0`). Всё остальное читается из удостоверенного бандла, потому что
всё в бандле покрыто удостоверением и потому стоит строго больше всего, что
вписано в форму.

## 9 · Что происходит дальше

Подробности, включая каждый код: [Попасть в каталог §что происходит после
отправки](5-publish/get-listed.md#3--что-происходит-после-отправки). Коротко:
панель показывает состояние заявки, и те же моменты приходят к вам
уведомлениями — на подтверждённый адрес почты вашего аккаунта Minice и в
панель, а в Telegram — только если вы его подключили.

| Исход | Значит | Кто участвует |
|---|---|---|
| **Опубликовано** | Закоммичено, затем в подписанном каталоге | никто |
| **Отложено** | Всё прошло; публикуется само в то время, которое показывает панель | никто |
| **Удержано** | Решение, которое реестр не вправе принимать автоматически | модератор, в панели |
| **Отказано** | Проверка не пройдена | вы: исправьте, затем Recheck в панели или новый тег, как говорит код |

**Первый листинг всегда удерживается для человека** — одно из ровно трёх
событий, которым он нужен, наряду с впервые запрошенным высокорисковым
разрешением и сменой репозитория или привязки. Модератор одобряет или
отклоняет его в панели; пока вы ждёте, вы ничего не делаете. Никакое одобрение
не сокращает задержку, а правила публикует `docs/POLICY.md` реестра.

## 10 · Каждый релиз после этого

Ничего. Поставьте тег, и остальное сделает CI; реестр сам замечает новый тег
внесённого в каталог плагина, и панель показывает его состояние.
...
печатает SHA переиспользуемых workflow, которые бот примет в аттестации —
их два с тех пор, как тег переехал 2026-08-19: коммит, на который указывает
`plugin-release/v1`, и тот, на который он указывал раньше. С 2026-09-20
каталог, который получают клиенты, подписан этим ключом; копии,
зафиксированные в `main` реестра, `registry/v1/index.json` и
`revocations.json`, несут `"signatures": []` намеренно, и ни один клиент их
не читает. **Всё ещё недостающее звено — подписанный список отзыва на
Pages**, так что отзыв пока не применяется. Ничто здесь не
