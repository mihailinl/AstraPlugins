...
**Скачайте бинарник.** Без тулчейна, и заканчивается печатью версии:
...
Положите извлечённый `astra-plugin` куда-нибудь в свой `PATH`, например
`~/.local/bin`. Берите архив **musl** на любом Linux — архиву gnu нужен glibc
2.39 или новее, которого нет в Ubuntu 22.04, Debian 12 и RHEL 9. На Windows
берите `astra-plugin-0.2.1-windows-x64.zip`. `--ignore-missing` важен:
`SHA256SUMS.txt` перечисляет все три архива, и без него `sha256sum` выходит
с 1 из-за двух, которые вы не скачали.
...
Скачанный бинарник `0.2.1` печатает `astra-plugin 0.2.1`.

**Либо соберите из исходников** — путь для macOS и ARM Linux, где архива
пока нет. Занимает несколько минут:
...
Здесь число — заполнитель намеренно: `--git` собирает тот коммит, который
`master` несёт в момент запуска, так что печатается версия этого коммита, а
не выбранная вами. Из клонированного репозитория `cargo install --path
astra-plugin-cli --locked` делает то же самое.

**Сборке из исходников нужен Rust 1.85 или новее и `protoc` в PATH.** Без
`protoc` сборка остановится на ``Could not find `protoc` ``. Поставьте его
через `apt install protobuf-compiler`, `pacman -S protobuf`, `brew install
protobuf` или `winget install Google.Protobuf`, затем выполните строку
заново. Скачанному бинарнику не нужно ни то, ни другое.

**При сборке из исходников номер версии не может сказать вам, что эта
сборка хорошая, а `0.2.0` — не плохая.** Скачанный бинарник `0.2.1` собран
из тега `cli-v0.2.1` и уже несёт исправление ниже; остальная часть этого
абзаца — про `--git`. `init-ci` раньше закреплял *объект* тега там, где
GitHub требует коммит, и первый `git push --tags` плагина умирал на этом.
...
напечатанный пин, сверив его с `git ls-remote
https://github.com/mihailinl/AstraPlugins.git refs/tags/plugin-release/v1
'refs/tags/plugin-release/v1^{}'` — берите снятую (`^{}`) строку, если она
есть. Это сравнение — всё, что осталось от бага: с 2026-08-11 по 2026-08-19
`plugin-release/v1` был аннотированным тегом, и сборка старше `5b8ab22`
печатала вместо коммита его объект тега
`dc1a044876926e9cf1170f034e2eab533ec07641`; сегодня тег лёгкий, так что
этот SHA больше никто не печатает.
Длинная версия:
[установка CLI](../install-cli.md#баг-который-ломает-первый-релиз-и-как-понять-есть-ли-в-вашей-сборке-исправление).

Одна оговорка, которая вас не заблокирует: CLI не на crates.io, так что
`cargo install astra-plugin-cli` падает — она зависит от завендоренного
крейта по пути, который cargo не упакует. Скачайте архив или соберите из
git. Все подробности, включая как сверить скачанное с его Sigstore-пакетом:
...
**Одна вещь, которую нужно сделать перед этой заявкой** — единственный шаг,
чьё отсутствие получает отказ для правильного плагина: привяжите репозиторий к
своему аккаунту Minice. Выпустите токен в панели на
https://astra.minice.ai/plugins, и одна команда запишет его в
`.well-known/astra-plugin-owner`, который нужно закоммитить в дефолтную ветку:
...
Именно так реестр узнаёт, какой аккаунт говорит от имени листингуемого
репозитория, — аттестация сборки доказывает, откуда взялся бандл, а не то, кто
вы. Подробности: [Попасть в каталог — Привяжите репозиторий](../5-publish/get-listed.md#привяжите-репозиторий).
