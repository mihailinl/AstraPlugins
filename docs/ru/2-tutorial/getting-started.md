...
чьё отсутствие получает отказ для правильного плагина: закоммитьте
`.well-known/astra-plugin-owner` в дефолтную ветку своего репозитория с
вашим логином GitHub внутри. Именно так реестр устанавливает, что вы
контролируете листингуемый репозиторий, — аттестация сборки доказывает,
откуда взялся бандл, а не то, кто вы. Две строки, вместе с вашим первым
пушем:
...
Подробности и почему автоматические проверки не могут ответить за вас:
[Попасть в каталог §2](../5-publish/get-listed.md#2--докажите-что-вы-контролируете-репозиторий).
