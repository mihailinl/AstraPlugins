> Переклад. Джерело істини — [docs/en](../../en/reference/parity.md); за розбіжності відповідає англійська версія. Англійська сторінка згенерована `tools/parity/gen.py` з `spec/hooks.yaml`; ця перекладна копія не генерується автоматично і може відстати при зміні специфікації.
