...
повірити, запустіть `astra-plugin init-ci` і звірте надрукований пін з
`git ls-remote https://github.com/mihailinl/AstraPlugins.git
refs/tags/plugin-release/v1 'refs/tags/plugin-release/v1^{}'` — беріть
зняту (`^{}`) рядок, якщо вона є. Це порівняння — все, що лишилося від
бага: з 2026-08-11 до 2026-08-19 `plugin-release/v1` був анотованим тегом,
і збірка старша за `5b8ab22` друкувала замість коміту його об'єкт тега
`dc1a044876926e9cf1170f034e2eab533ec07641`; сьогодні тег легкий, тож цього
SHA більше ніхто не друкує. Довга
версія:
...
відсутність отримує відмову для правильного плагіна: прив'яжіть репозиторій до
свого акаунта Minice. Випустіть токен у панелі на
https://astra.minice.ai/plugins, і одна команда запише його в
`.well-known/astra-plugin-owner`, який треба закомітити в дефолтну гілку:
...
Саме так реєстр дізнається, який акаунт говорить від імені репозиторію, що
лістингується, — атестація збірки доводить, звідки взявся бандл, а не хто ви.
Подробиці: [Потрапити до каталогу — Прив'яжіть репозиторій](../5-publish/get-listed.md#привяжіть-репозиторій).
