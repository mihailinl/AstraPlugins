...
| `#[action(label = "…", icon = "…")]` | Крок у редакторі команд, і гліф, який він там носить |
...
**Гліф, який носить крок.** `icon` у `#[action]` і `with_icon_svg` у зібраного
вручну `ActionTypeDef` або `TriggerTypeDef` — це те, що користувач бачить на
полотні, у палітрі та в заголовку налаштувань кроку. Передавайте цілу
SVG-розмітку, а не рядок шляху: задайте `viewBox` і не вказуйте `width`/`height`
(розмір обирає Astra, а ці три поверхні не однакового розміру) та малюйте через
`currentColor`, щоб гліф ішов за тим, чим кожна поверхня підфарбовує свої
піктограми. Розмітка проходить очищення перед відмальовуванням — скрипти,
обробники подій і зовнішні посилання вирізаються, а не відхиляються, — тож
лишайтеся в межах простого штрихового малюнка. Не задавайте її, і крок усе одно
намалюється: він відкотиться до власної піктограми плагіна, а потім до загальної
позначки плагіна.
