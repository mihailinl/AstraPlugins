...
**Частково в силі:** ланцюг підпису, описаний тут, специфікований,
реалізований на обох кінцях і заякорений через каталог, але поки не через
список відкликання — див. §0.1, перш ніж
...
* Один наслідок досі в силі, і його не можна замовчувати. Pages досі віддає
  зафіксований непідписаний список відкликання, поки реєстр не ввімкне там
  підписаний, а оскільки `verify_revocations_document` строга (§6.4),
  **непідписаний список відкликання відхиляється, тож застосування
  відкликання поки не діє** — `RevocationFreshness::NotEnforced`, поки
  список, валідний за підписом, не буде отриманий хоча б раз.
...
машині користувача. Залишилася половина списку відкликання: підписаний
список на Pages.
...
`downloads` і `stars` у підписаному каталозі завжди `0` і залишаються `0`:
лічильник ніколи не підписується і ніколи не входить у рішення про довіру,
встановлення чи оновлення. **Змінено 2026-09-27 (ROLL-47, рядок B1):** раніше ця
сторінка додавала, що реєстр нічого не рахує. Від деплою встановлень сервісу
плагінів встановлення й оновлення рахуються як агрегати по плагіну і
публікуються поряд із каталогом, у `https://registry.minice.ai/stats/v1/stats.json`
(`astra.plugins.stats/1`: `generated_at`, `catalogue_serial` і для кожного
плагіна `installs`, `updates` та, починаючи з п'яти оцінок, гістограма
`ratings`). Жоден член цього документа не називається `downloads` чи `stars`.
