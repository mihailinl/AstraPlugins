...
Коротка версія — для форми issue, якою сьогодні подають перший лістинг:
...
**У формі issue перша заявка на лістинг завжди утримується для людини** —
це одна з рівно трьох подій, яким там вона потрібна, поряд з вперше
запитаним високоризиковим дозволом і зміною репозиторію. 48 годин —
опублікований SLA для всіх них.

**У панелі плагінів Astra, від перемикання реєстру, її не утримують.** Реліз,
що проходить кожну автоматичну перевірку, публікується одразу, з позначкою, що
модератори Astra його не переглядали. Починаючи з релізу Astra, який додає
позначку, користувачі бачать попередження, доки модератор не перегляне цю
версію; старіші релізи Astra не показують нічого. Там на модератора чекає лише
зміна власника лістингу, який уже існує. Див. [Потрапити до каталогу §на шляху
через панель](5-publish/get-listed.md#на-шляху-через-панель-від-перемикання).
...
перевірка безпеки, і версія, яку модератор позначив переглянутою, теж ні: це
лише означає, що модератор прочитав цю версію, і нічого більше. Реєстр каже
про це у власній політиці.
