...
поведінки — посилання, яке `publish` друкував для вже внесеного до каталогу
плагіна: відтоді воно називало форму реєстру для цього замість порожнього
issue, який реєстр вимкнув. Цієї форми і прапорця, який її відкривав, немає
починаючи з CLI 0.5.0: реєстр сам помічає новий тег.
...
Повідомлене закріплення зобов'язане бути тим комітом, на який вказує
`plugin-release/v1`, а сказати, який це коміт, може лише віддалений
репозиторій: `git ls-remote https://github.com/mihailinl/AstraPlugins.git
refs/tags/plugin-release/v1 'refs/tags/plugin-release/v1^{}'` — беріть
зняту (`^{}`) рядок, якщо вона є. Якщо закріплення розходиться,
...
застарілий SHA, поки ви його не перезапустите. Відмова, за якою названо цей
розділ, тепер історія, і вона датована: з 2026-08-11 до 2026-08-19
`plugin-release/v1` був анотованим тегом, і збірка старша за коміт
`5b8ab22` повідомляла його *об'єкт тега*
`dc1a044876926e9cf1170f034e2eab533ec07641`, який GitHub відхиляє, —
сьогодні тег легкий, тож повідомити цей SHA не може жодна збірка.
