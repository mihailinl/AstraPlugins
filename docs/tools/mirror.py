...
Digests were not enough either, and the sixth hole was not drift at all. It was
a sentence that was true in one file and got copied into twenty where it is the
opposite of true.

Ten English pages are written by `tools/docgen/gen.py` and `tools/parity/gen.py`
and carry a `GENERATED by … — DO NOT EDIT` banner. **No generator writes a
translation of any of them.** Every locale's copy of those ten is kept by hand,
and four of the six said exactly that in their own words — *the English page is
GENERATED by `tools/docgen/errors.py`; this translation is a hand-maintained
snapshot of it, not another generated copy.* `docs/ja` and `docs/zh` pasted the
English banner in verbatim instead, into all ten pages each.

> Twenty pages told a translator not to edit the only file they were supposed to
> edit.

That direction is the quiet one, and it is quiet in the worst way. A translator
who obeys the banner does nothing, commits nothing, and leaves an untranslated
page behind with no record of why — which is the symptom the heading rule above
exists to catch, arriving with its own cause already erased. The same twenty
pages were also the only twenty that did NOT name the generator their original
comes from, so a reader got the false instruction and not the true one. Nothing
checked either. So banners are compared:

* `GENERATED` below names every generated page AND the generator that writes it.
  The banner belongs on exactly those pages in `docs/en`, and on no page in any
  other locale. Both directions, per locale, over the whole tree.
* A banner must name a generator that is a file in this repository, and the one
  `GENERATED` declares. A banner naming a generator that was renamed or deleted
  is a page claiming a provenance nobody can check.
* Every translation of a generated page must name that generator in its leading
  note. Deleting a wrong banner and putting nothing in its place leaves the
  reader knowing less than before: no longer lied to about who owns the page,
  and still not told.
* `GENERATED_COUNT` must equal the list's length — the same ratchet
  `GRANDFATHERED_CEILING` is, for the same reason. A page joining or leaving the
  generated set costs a number in the diff, with a reviewer looking at it.

`GENERATED` is a declaration, not an exemption, so its rows carry a generator
rather than a reason. The generator's existence does for this list what a
reason's exit condition does for the one below: it is the part that makes an
entry refuse to rot in place.

Only the English half of this was covered before, and by a different job.
`tools/docgen/gen.py --check` and `tools/parity/gen.py --check` pin those ten
pages byte for byte, banner included — but both need a Rust toolchain and a
built CLI, because `reference/cli.md` is generated by RUNNING `astra-plugin
--help`. This script needs neither, so the English direction is asserted here
too rather than borrowed from a job that may not have run. The translations were
...
# ── the generated pages, as data ────────────────────────────────────


@dataclass(frozen=True)
class Generated:
    """One English page a generator writes, and the generator that writes it.

    `generator` is a repository-relative path and must be a real file: a row
    naming a script that was renamed or deleted asserts a provenance nobody can
    check any more. It is also matched against the generator the page's own
    banner names, so the declaration and the artefact cannot drift apart
...
#: Every page under `docs/en` that a generator writes. Two generators produce
#: all ten: `tools/docgen/gen.py` renders `reference/`, delegating one page each
#: to `cli.py`, `manifest.py`, `protocol.py`, `errors.py` and `parity.py` —
#: which is the name each of those banners carries, not `gen.py` — and
#: `tools/parity/gen.py` writes the flat `parity.md` and the three `hooks/`
#: tables. Nothing writes a translation of any of them.
GENERATED = (
    Generated("hooks/python.md", "tools/parity/gen.py"),
    Generated("hooks/rust.md", "tools/parity/gen.py"),
    Generated("hooks/typescript.md", "tools/parity/gen.py"),
    Generated("parity.md", "tools/parity/gen.py"),
    Generated("reference/README.md", "tools/docgen/gen.py"),
    Generated("reference/cli.md", "tools/docgen/cli.py"),
    Generated("reference/errors.md", "tools/docgen/errors.py"),
    Generated("reference/manifest.md", "tools/docgen/manifest.py"),
    Generated("reference/parity.md", "tools/parity/gen.py"),
    Generated("reference/protocol.md", "tools/docgen/protocol.py"),
)

#: Must equal `len(GENERATED)` exactly, in both directions — the same ratchet
#: `GRANDFATHERED_CEILING` is. A page becoming generated, or ceasing to be,
#: changes what a whole tier of this tree may be hand-edited, and that should
#: cost a number in the diff rather than a silently longer tuple.
GENERATED_COUNT = 10
...
#: The banner pass reads every page in every locale, English included — 266
#: today. A banner rule that reached nothing would report no banner anywhere and
#: call the tree clean, which is the exact shape of the two guards this
#: repository has already shipped blind.
FLOOR_BANNER_PAGES = 200
...
#: Any HTML comment, body captured. Banners are read as COMMENTS rather than as
#: a text scan for the words, so a page that needs to SHOW a banner can quote
#: one — and so that prose discussing the convention is not mistaken for a page
#: carrying it.
COMMENT_RE = re.compile(r"<!--(.*?)-->", re.S)

#: The words that make a comment an instruction rather than a note. This is the
#: literal `tools/docgen/common.py`'s `banner()` emits and `tools/parity/gen.py`
#: repeats.
BANNER_MARK = "DO NOT EDIT"

#: The generator a banner names, as a path. The same string in every language,
#: which is what makes it comparable at all.
BANNER_GENERATOR = re.compile(r"GENERATED by (\S+)")


def banners(path: Path) -> list[str | None]:
    """Every DO-NOT-EDIT banner on a page, as the generator each one names.

    Read out of the page's prose, fences stripped, for the reason `digests` and
    `headings` are: a fenced block may quote a banner the page does not carry.
    `None` in the result is a banner that says DO NOT EDIT and names no
    generator — an instruction with nobody behind it.
    """
    text = "\n".join(raw for _, raw in prose_lines(path))
    found: list[str | None] = []
    for body in COMMENT_RE.findall(text):
        if BANNER_MARK not in body:
            continue
        m = BANNER_GENERATOR.search(body)
        found.append(m.group(1) if m else None)
    return found


def leading_note(path: Path) -> str:
    """The translator's note at the top of a page: its blockquote, before the first heading.

    Every locale writes one and each writes it differently — `> **Übersetzung.**
    …`, `> Перевод. …`, `> **本文档为翻译版本。** …`. What is compared is never
    its words, only whether a path this repository owns appears inside it.
    """
    out: list[str] = []
    for _, raw in prose_lines(path):
        body = raw.strip()
...
def _page_banners(loc: str, rel: str, page: Path, generator: str | None) -> int:
    """One page's banners, against what the generated set says about that page.

    Split out of `check_banners` so the loop can attribute findings to a locale:
    a per-locale `ok` line printed over a locale that has findings is the
    failure this whole file was rewritten for, and it is not going to come back
    through a counter that only totals.
    """
    bad = 0
    found = banners(page)
    ...
    if loc != "en":
        if found:
            named = ", ".join(g or "(no generator named)" for g in found)
            owner = (
                f"{generator} writes docs/en/{rel} and writes no translation "
                f"of it"
                if generator
                else f"no generator writes docs/en/{rel} either"
            )
            print(
                f"BANNER   docs/{loc}/{rel}  — carries a {BANNER_MARK} banner "
                f"naming {named}, and nothing generates this file. {owner}; "
                f"this page is maintained by hand, and the banner tells the "
                f"one person who is supposed to edit it not to.\n"
                f"           That instruction is obeyed in silence: a "
                f"translator who follows it does nothing, commits nothing, and "
                f"leaves a page that looks untranslated for a reason nobody "
                f"will ever find. Delete the banner, and say what is true in "
                f"its place — docs/de and docs/ru word it as *the English page "
                f"is generated by X; this translation is a hand-kept snapshot "
                f"of it, not another generated copy*"
            )
            bad += 1
        if generator and generator not in leading_note(page):
            print(
                f"BANNER   docs/{loc}/{rel}  — docs/en/{rel} is written by "
                f"{generator} and this translation's note does not name it. A "
                f"reader has no way to tell a page re-derived from source from "
                f"one somebody wrote, and a translator has no way to tell that "
                f"their copy goes stale the next time that script runs.\n"
                f"           Name the generator in the note at the top of the "
                f"page, in this language, the way the other locales do. A path "
                f"is the same word everywhere, which is why it is the part "
                f"that can be checked. Deleting a wrong banner and putting "
                f"nothing in its place leaves the reader knowing less than "
                f"before"
            )
            bad += 1
        return bad
    ...
    if generator is None:
        if found:
            named = ", ".join(g or "(no generator named)" for g in found)
            print(
                f"BANNER   docs/en/{rel}  — carries a {BANNER_MARK} banner "
                f"naming {named}, and GENERATED does not list this page.\n"
                f"           Either it really is generated and the row is "
                f"missing — add it and raise GENERATED_COUNT — or the banner "
                f"was copied onto a hand-written page, and it is now telling "
                f"its author not to touch it"
            )
            bad += 1
        return bad
    ...
    if not found:
        print(
            f"BANNER   docs/en/{rel}  — is written by {generator} and carries "
            f"no {BANNER_MARK} banner. Without it the page invites the one "
            f"edit that cannot survive: a hand fix here is discarded, without "
            f"a word, by the next run of the generator — and the person who "
            f"made it has every reason to believe it landed.\n"
            f"           Restore the banner in {generator} and re-run it. If "
            f"this page stopped being generated, drop its GENERATED row and "
            f"lower GENERATED_COUNT"
        )
        bad += 1
        return bad
    ...
    if len(found) != 1:
        print(
            f"BANNER   docs/en/{rel}  — carries {len(found)} {BANNER_MARK} "
            f"banners and one generator writes it. Two banners are two claims "
            f"of ownership, and a reader told to change the source cannot tell "
            f"which source"
        )
        bad += 1
    for named in found:
        if named != generator:
            print(
                f"BANNER   docs/en/{rel}  — its banner names "
                f"{named or '(no generator)'} and GENERATED says {generator}. "
                f"One of the two is out of date, and the banner is the one a "
                f"reader acts on: it is the line that tells them which file to "
                f"change instead of this one"
            )
            bad += 1
    return bad


def check_banners(en: set[str]) -> tuple[int, int, Counter]:
    """The DO-NOT-EDIT banner, against the set of pages that are actually generated.

    Both directions, in every locale: present where a generator writes the file,
    absent where a human maintains it. Returns `(findings, pages examined,
    findings per locale)`.
    """
    bad = 0
    examined = 0
    ...
    if len(GENERATED) > GENERATED_COUNT:
        print(
            f"COUNT    docs/tools/mirror.py  — GENERATED has {len(GENERATED)} "
            f"row(s) and GENERATED_COUNT is {GENERATED_COUNT}. A page joining "
            f"the generated set is one more page nobody may hand-edit and "
            f"{len(TRANSLATIONS)} more translations that have to be re-cut by "
            f"hand every time it is re-rendered; raise the literal to "
            f"{len(GENERATED)} in the same commit, where a reviewer sees the "
            f"number move"
        )
        bad += 1
    elif len(GENERATED) < GENERATED_COUNT:
        print(
            f"COUNT    docs/tools/mirror.py  — GENERATED has {len(GENERATED)} "
            f"row(s) and GENERATED_COUNT is still {GENERATED_COUNT}. A page "
            f"left the generated set; lower the literal to {len(GENERATED)} so "
            f"the room it freed cannot be taken without anybody deciding to "
            f"give it"
        )
        bad += 1
    ...
    for g in GENERATED:
        if g.page in declared:
            print(f"COUNT    {g.page}  — listed twice in GENERATED")
            bad += 1
        declared[g.page] = g.generator
        if g.page not in en:
            print(
                f"STALE    {g.page}  — docs/en/{g.page} does not exist. Delete "
                f"this GENERATED row and lower GENERATED_COUNT"
            )
            bad += 1
        if not (ROOT / g.generator).is_file():
            print(
                f"STALE    {g.page}  — GENERATED says {g.generator} writes it, "
                f"and that file is not in this repository. A generator that "
                f"moved leaves its page marked {BANNER_MARK} with nothing "
                f"behind the instruction: point the row at the script that "
                f"writes the page now, or drop the row and lower "
                f"GENERATED_COUNT"
            )
            bad += 1
    ...
    for loc in ("en",) + tuple(TRANSLATIONS):
        for rel in sorted(en):
            ...
            if not page.is_file():
                continue
            examined += 1
            here = _page_banners(loc, rel, page, declared.get(rel))
            bad += here
            per_locale[loc] += here

    return bad, examined, per_locale
...
def main() -> int:
    ...
    marked, banner_pages, marked_here = check_banners(en)
    ...
    for loc in TRANSLATIONS:
        ...
        skeleton += heads
        values += marks
        if got == en and not (here or rows or heads or marks or marked_here[loc]):
            print(f"ok       docs/{loc}/  — {len(got)} page(s), same set, same "
                  f"samples, same table names, same heading skeleton, same "
                  f"generated values, and no banner it has no generator for")
    ...
    for name, saw, least in (
        ...
        ("banner pages", banner_pages, FLOOR_BANNER_PAGES),
    ):
        if saw < least:
            print(
    ...
    total = (bad + drift + tables + skeleton + values + exemptions + pinned
             + marked + floor)
    ...
    print(f"mirror: floor — {locales_seen} locale(s), {pages_compared} page(s), "
          ...
          f"{banner_pages} page(s) read for banners, "
          f"{len(GRANDFATHERED)}/{GRANDFATHERED_CEILING} grandfathered, "
          f"{len(GENERATED)}/{GENERATED_COUNT} generated")
