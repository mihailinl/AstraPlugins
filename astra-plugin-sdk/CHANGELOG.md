...
- `PluginContext::scoped_host` and `for_invocation`, which wrap the host in a
  `CausalHost`. The cause rides inside the `Arc<dyn Host>` rather than in a
  task-local, because the shipped idiom clones the host into a `tokio::spawn`
  and a task-local does not cross that.
